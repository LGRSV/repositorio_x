...
for r in r0:
    ...
    alvo.append({'ss':str(g(r,11) or '').strip(),'trafo':cods[0] if cods else '','trafo2':cods[1] if len(cods)>1 else '',
      ...
      'cl_info':g(r,8),'cl_fis':g(r,9),'cl_ts':g(r,10),'obra_orig':g(r,26)})

hS,IS,rS=load(U+'8cfc359b-Trafo.xlsx','BASE_SS_OS')
ss2row={}
for r in rS:  # fica com a linha MAIS preenchida (a aba repete 1.444 SS com uma linha quase vazia)
    k=str(g(r,IS['NUMERO_SS']) or '').strip()
    if k and (k not in ss2row or cheia(r)>cheia(ss2row[k])): ss2row[k]=r
...
pc=json.load(open(B+'public/passos-critica.json')); por_ss=pc.get('por_ss',{}); por_oc=pc.get('por_oc',{})
hist=json.load(open(B+'public/historico-ativo.json')).get('por_ativo',{})
...
S=[('OS','NUMERO_OS'),('Obra','NUM_OBRA'),('Esquema de serviço','ESQUEMA'),('Situação da SS','SITUACAO_SS'),
   ...
   ('Origem da SS','ORIGEM_SS'),('Defeito da SS','DEFEITO_SS'),('Origem da OS','ORIGEM'),('Defeito da OS','DEFEITO'),
   ('Equipe','COD_EQUIPE'),('Solicitante','SOLICITANTE'),('Clientes','QTD_CLIENTES'),('Rural/urbano','RURA_URBANO'),
   ('Tipo da SS','TIPOSS'),('Nº interrupção','NUM_INT'),('Coord X','COORD_X'),('Coord Y','COORD_Y')]
T=[('Série retirada','NS_RETIRADO'),('Série instalada','NS_INSTALADO'),('Tombamento retirado','TOMBAMENTO_RETIRADO'),
   ...
   ('Reformadora instalado','REFORMADORA_INSTALADO')]
...
AI=[('Obra status','DSC_STATUS'),('Obra status AIC','STATUS AIC'),('Obra AIC','AIC'),('Obra projeto SIGCO','NUM_PROJETO_SIGCO'),
    ...
    ('Obra orçado','VAL_TOTAL_ORCADO'),('Obra realizado','TOTAL_REALIZADO'),('Obra empreiteira','EMPREITEIRA_PROJETO')]
CR=[('Crítica ocorrência','oc_num'),('Crítica início','oc_ini'),('Crítica fim','oc_fim'),('Crítica duração (h)','oc_dur_h'),
    ('Crítica consumidores','oc_cons'),('Crítica papel do trafo','oc_papel'),('Crítica elemento com problema','oc_prob_ele'),
    ('Crítica tipo','oc_tipo'),('Crítica causa','oc_causa'),('Crítica subcausa','oc_sub'),('Crítica observação','oc_obs'),
    ('Crítica distância da SS (h)','oc_dist_h'),('Interrupção na janela','int_na_janela'),('Ocorrências no ativo','ocorrencias_ativo')]
TM=[('TMAE atendimento','at_num'),('TMAE início','at_ini'),('TMAE fim','at_fim'),('TMAE equipe','at_equipe'),
    ('TMAE causa','at_causa'),('TMAE subcausa','at_sub'),('TMAE observação','at_obs'),('TMAE TMA','at_tma'),
    ('TMAE fora da janela','at_fora_da_janela'),('Atendimentos no ativo','atendimentos_ativo')]
DE=[('Categoria do site','categoria'),('Decisão','decisao_base'),('Regra','regra_base'),('Causa apurada','causa_base'),
    ('Motivo apurado','motivo_base'),('Cascata','cascata'),('Expurgo','expurgo'),('Gatilho do expurgo','expurgo_gatilho'),
    ('Confiança da leitura','confianca_leitura'),('Material conferido','material_conferido'),('Trafos no material','trafos_material'),
    ('Potência subiu','potencia_subiu'),('Delta de potência','potencia_delta'),('Idade da SS (dias)','idade_ss'),('Narrativa','narrativa')]

out=[]
for a in alvo:
    L={'SS':a['ss'],'Transformador':a['trafo'],'2º código no arquivo':a['trafo2'],'Alimentador':a['alim'],
       'Localidade':a['loc'],'Polo':a['polo'],'Regional':a['reg'],'Potência Infotrafo':a['pot_info'],
       'Potência FIS (arquivo)':a['pot_fis'],'Potência TS':a['pot_ts'],'Clientes Infotrafo':a['cl_info'],
       'Clientes FIS (arquivo)':a['cl_fis'],'Clientes TS':a['cl_ts']}
    r=ss2row.get(a['ss'])
    L['Achada na base de SS/OS de 10/09']='sim' if r else 'não'
    for n,c in S+T+C: L[n]=d(g(r,IS.get(c))) if r else ''
    ...
    for n,c in [('OS_STATUS nº','NUMERO_OS'),('OS_STATUS esquema','DESCESQUEMA'),('OS_STATUS situação','SITUACAO'),
                ('OS_STATUS criação','DATA_CRIACAO'),('OS_STATUS início exec.','INICIO_EXEC'),('OS_STATUS fim exec.','FIM_EXEC'),
                ('OS_STATUS equipe executora','EQUIPE_EXECUTORA'),('OS_STATUS origem exec.','ORIGEM_EXEC'),('OS_STATUS impedimento','IMPEDIMENTO')]:
        L[n]=d(g(pick,IO.get(c))) if pick else ''
    ob=nz(L.get('Obra') or a['obra_orig']); ar=aic.get(ob)
    L['Obra achada no AIC de 12/09']='sim' if ar else ('não' if ob else '')
    for n,c in AI: L[n]=d(g(ar,IA.get(c))) if ar else ''
    mm=mat.get(ob,[])
    ...
    # ---- SITE / CRITICA
    sv=site.get(a['ss'])
    L['Achada no site (esteira 1.582)']='sim' if sv else 'não'
    for n,c in CR+TM+DE: L[n]=(' '.join(str(sv.get(c,'')).split())[:900] if sv else '')
    ps=por_ss.get(a['ss'])
    L['Passos da Crítica — papéis']=', '.join(ps.get('papeis',[])) if ps else ''
    L['Passos — nº de passos']=ps.get('n','') if ps else ''
    L['Passos — só manobra']=('sim' if ps.get('so_manobra') else 'não') if ps else ''
    L['Passos — ausente nos passos']=('sim' if ps.get('ausente_nos_passos') else 'não') if ps else ''
    L['Passos — não restabelecido']=('sim' if ps.get('nao_restabelecido') else 'não') if ps else ''
    L['Passos — socorrido']=('sim' if ps.get('socorrido') else 'não') if ps else ''
    hv=hist.get(a['trafo']) or hist.get(nz(a['trafo']))
    L['Histórico do ativo — SS no período']=len(hv.get('ss',[])) if hv else ''
    e=exp.get(a['ss'])
...
print('colunas:',len(out[0]))
for k in ['Achada na base de SS/OS de 10/09','Achado no cadastro FIS','Obra achada no AIC de 12/09','Achada no site (esteira 1.582)','Está na base de expurgos']:
    print(f'  {k:38s}',dict(collections.Counter(str(o[k]) for o in out)))
pre=lambda k: sum(1 for o in out if str(o.get(k,'')).strip() not in ('','None'))
print('\nPREENCHIMENTO:')
for k in ['OS','Situação da SS','Série retirada','Série instalada','Potência retirada','Potência instalada','Provável motivo do defeito','Crítica causa','Crítica papel do trafo','TMAE causa','Categoria do site','Passos da Crítica — papéis']:
    print(f'  {k:32s} {pre(k)}/16')
