...
def main():
    fonte = "todos_full.json" if os.path.exists(os.path.join(BASE, "todos_full.json")) else "todos.json"
    casos = {c["ss"]: c for c in json.load(open(os.path.join(BASE, fonte), encoding="utf-8"))}
    print(f"dossiê: {fonte}")
    vereditos, faltando = {}, []
    for i in range(1, 6):
        ...
        for v in d:
            vereditos[v["ss"]] = v
    rk = os.path.join(BASE, "saida_recheck.json")
    relidos = set()
    if os.path.exists(rk):
        for v in json.load(open(rk, encoding="utf-8")):
            vereditos[v["ss"]] = v
            relidos.add(v["ss"])
        print(f"releitura com texto completo aplicada a {len(relidos)} SS")
    ...
    for ss, c in casos.items():
        ...
        linhas.append({
            ...
            "Relido com texto completo": "SIM" if ss in relidos else "",
            "O que o texto completo acrescentou": v.get("o_que_o_texto_completo_acrescentou") or "",
