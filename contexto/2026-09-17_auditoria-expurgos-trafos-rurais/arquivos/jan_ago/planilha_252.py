...
L = json.load(open(os.path.join(SC, "as_252.json"), encoding="utf-8"))
...
COLS = [("Categoria", "categoria_padrao"), ("Conta no indicador", "_conta"),
...
ws.append(["As 252 SS de substituição de transformador que ficaram fora do Infotrafo"])
...
for l in L: g[l["categoria_padrao"]].append(l)
for k in ORDEM:
    if g.get(k):
...
for m in sorted({l["_mes"] for l in L}):
    ...
    ws.append([m, len(gg), sum(1 for l in gg if l["categoria_padrao"] == "QUEIMADO"),
               sum(1 for l in gg if l["categoria_padrao"] == "AVARIADO"),
               sum(1 for l in gg if l["conta_no_indicador"])])
ws.append([])
for t in ["Universo: 1.671 SS de substituição de transformador abertas de janeiro a agosto de 2026 (aba BASE SS_OS).",
          "Infotrafo: 1.444 SS, das quais 1.419 caem dentro dessas 1.671 e 25 estão só na aba maior. Sobram 252 fora.",
          "Das 252, 196 já tinham análise no site (janeiro a julho) e a categoria foi aproveitada sem reanálise.",
          "As outras 56 (23 de julho, 33 de agosto) foram lidas caso a caso: texto da SS, texto da OS, formulário de campo e busca na Crítica.",
          "A Crítica bruta de julho está perdida: as 23 SS de julho foram julgadas só por texto e campo, sem conferência na base de interrupção.",
...
saida = os.path.join(SC, "As_252_fora_do_Infotrafo.xlsx")
wb.save(saida)
print(saida, len(L))
